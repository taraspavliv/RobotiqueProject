...
#define LINES_TO_ANALYSE 3 //for vertical averaging
#define MINIMUM_BALL_SIZE 15 // if smaller it's probably a black dot
#define RGB_nb 3

#define DARK_PXL_THRS 10 //threshold for what is considered a black pixel
#define BALL_EDGE_THRS 2 //threshold for the edge of the ball detection (derivative)
#define BALL_AOV_MARGIN 10 //in pixels, ball should be BALL_AOV_MARGIN inside the angle of view to be considered fully seen

#define CAMERA_ANGLE_OF_VIEW 0.7854 //unit: rad, 45 degree
#define BALL_RADIUS_SEEN 18.735 //radius of the ball at the level of the camera, unit: mm
...
#define SPEED_CONVERTER 100 //converts the speed to something easier to work than mm/ms
...
static THD_FUNCTION(CaptureImage, arg) {
    ...
    //set grayscale image and look at the center line
...
static THD_FUNCTION(ProcessImage, arg) {
    ...
    while(true){
		...
		//gets the pointer to the array filled with the last image in grayscale
		img_buff_ptr = dcmi_get_last_image_ptr();

		//makes a first estimation of the edge of the ball by checking for black pixels
		ball_seen = first_ball_estimate(img_buff_ptr, main_lines_averaged, &left_ball_side, &right_ball_side);
		if(ball_seen){
			//better estimate by doing an edge (derivative) detection
			final_ball_estimate(main_lines_averaged, &left_ball_side, &right_ball_side);
			//estimates ball position based on self position and self angle and the edges of the ball on the camera
...
bool first_ball_estimate(uint8_t *img_buff_ptr, uint8_t* main_lines_averaged, uint16_t* left_ball_side_ref, uint16_t* right_ball_side_ref){
	...
	//for spatial averaging we set the limit cases
	...
	for(uint16_t i=2; i < PO8030_MAX_WIDTH-2; ++i){
		...
		for(uint8_t j=0; j< LINES_TO_ANALYSE;++j){
			//we make an averaging on x and y axis
		...
		if(main_lines_averaged[i] <= DARK_PXL_THRS){
			if(ball_seen == false){
				ball_seen = true;
...
void final_ball_estimate(uint8_t* main_lines_averaged, uint16_t* left_ball_side_ref, uint16_t* right_ball_side_ref){
	...
	//tries to push the left ball side until there is a clear edge
	...
	}//else the left side is too close to the border and the input data isn't as reliable

	//Similar logic for the right side, looking for an edge further to the right
	...
	}//else too close to right side (to have reliable data)

}

void update_ball_pos_mov(uint16_t left_ball_side, uint16_t right_ball_side){
	//sets the ball visibility based on right and left edges
	...
	float ball_current_estimate_mov[2] = {0, 0}; //the speed vector
	...
	if(ball_visibility != NONE){
		//calculation of the distance to the ball and the relative angle of this position to the epuck. If ball is right in front, the angle is 0
		...
		ball_current_estimate_pos[0] = (int16_t)(get_self_position()[0] +
										RADIUS_TO_CAMERA*cosf(DEG_TO_RAD(get_self_angle())) +
										distance_to_ball*cosf(relative_angle_to_ball_center + DEG_TO_RAD(get_self_angle())));
		ball_current_estimate_pos[1] = (int16_t)(get_self_position()[1] +
										RADIUS_TO_CAMERA*sinf(DEG_TO_RAD(get_self_angle())) +
										distance_to_ball*sinf(relative_angle_to_ball_center + DEG_TO_RAD(get_self_angle())));

		//low pass filter position to have less noise
		...
		ball_current_estimate_mov[0] = ((ball_position_filtered[0] - ball_last_filtered_position[0])*1.0 /
										ST2MS(chVTGetSystemTime() - last_systick))*SPEED_CONVERTER;
		ball_current_estimate_mov[1] = ((ball_position_filtered[1] - ball_last_filtered_position[1])*1.0 /
										ST2MS(chVTGetSystemTime() - last_systick))*SPEED_CONVERTER;

		//low pass filter movement vector
