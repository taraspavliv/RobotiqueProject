...
void imu_compute_units(void){
    ...
    *   TASK 10 : TO COMPLETE
    */
	imu_values.acceleration[X_AXIS] = (imu_values.acc_raw[X_AXIS]-imu_values.acc_offset[X_AXIS])/16384.*STANDARD_GRAVITY;
	imu_values.acceleration[Y_AXIS] = (imu_values.acc_raw[Y_AXIS]-imu_values.acc_offset[Y_AXIS])/16384.*STANDARD_GRAVITY;
	imu_values.acceleration[Z_AXIS] = (imu_values.acc_raw[Z_AXIS]-imu_values.acc_offset[Z_AXIS])/16384.*STANDARD_GRAVITY;


	imu_values.gyro_rate[X_AXIS] = DEG2RAD((imu_values.gyro_raw[X_AXIS]-imu_values.gyro_offset[X_AXIS])/131.);
	imu_values.gyro_rate[Y_AXIS] = DEG2RAD((imu_values.gyro_raw[Y_AXIS]-imu_values.gyro_offset[Y_AXIS])/131.);
	imu_values.gyro_rate[Z_AXIS] = DEG2RAD((imu_values.gyro_raw[Z_AXIS]-imu_values.gyro_offset[Z_AXIS])/131.);
}
...
void imu_compute_offset(messagebus_topic_t * imu_topic, uint16_t nb_samples){
	...
	int16_t imu_acc_off_X=0;
	int16_t imu_acc_off_Y=0;
	int16_t imu_acc_off_Z=0;

	int16_t imu_rad_off_X=0;
	int16_t imu_rad_off_Y=0;
	int16_t imu_rad_off_Z=0;

    for(uint16_t i=0; i< nb_samples; ++i){
    	messagebus_topic_wait(imu_topic, &imu_values, sizeof(imu_values));
    	imu_acc_off_X += imu_values.acc_raw[X_AXIS]/nb_samples;
    	imu_acc_off_Y += imu_values.acc_raw[Y_AXIS]/nb_samples;
    	imu_acc_off_Z += imu_values.acc_raw[Z_AXIS]/nb_samples;

    	imu_rad_off_X += imu_values.gyro_raw[X_AXIS]/nb_samples;
    	imu_rad_off_Y += imu_values.gyro_raw[Y_AXIS]/nb_samples;
    	imu_rad_off_Z += imu_values.gyro_raw[Z_AXIS]/nb_samples;
    }

    imu_values.acc_offset[X_AXIS] = imu_acc_off_X;
    imu_values.acc_offset[Y_AXIS] = imu_acc_off_Y;
    imu_values.acc_offset[Z_AXIS] = imu_acc_off_Z + 16384; // still show 1g acc

    imu_values.gyro_offset[X_AXIS] = imu_rad_off_X;
    imu_values.gyro_offset[Y_AXIS] = imu_rad_off_Y;
    imu_values.gyro_offset[Z_AXIS] = imu_rad_off_Z;
}
