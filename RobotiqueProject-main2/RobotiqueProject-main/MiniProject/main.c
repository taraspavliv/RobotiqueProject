...
void attack_FSM(bool role_changed){
	...
	case A_FOCUSING:{
		if(get_ball_visibility() == FULL ){
			...
			/*if(get_angle_difference(angle_to_ball, get_self_angle())  < ATTACKER_MIN_FOCUS_ANGLE && get_angle_difference(angle_to_ball, get_self_angle()) > -ATTACKER_MIN_FOCUS_ANGLE){
				attacker_state = SHOOTING;
				break;
			}*/
