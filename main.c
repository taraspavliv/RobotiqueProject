...
static THD_FUNCTION(ThdFrontLed, arg) {
    ...
    while(1){
        ...
        palTogglePad(GPIOD, GPIOB_LED_BODY);
        chThdSleepUntilWindowed(time, time + /*MS2ST(500)*/ 500);
    }
}
...
void show_gravity(imu_msg_t *imu_values){
    ...
    *   TASK 11 : TO COMPLETE
    */
    if ( imu_values->acceleration[X_AXIS]>3 ){
    	palClearPad(GPIOD, GPIOD_LED7); //led7 port d
    }else {
